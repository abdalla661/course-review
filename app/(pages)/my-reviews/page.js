...
//       } else {
//         console.error("❌ Invalid student object in localStorage");
//         router.push("/login");
//       }
//     } else {
//       router.push("/login");
...
//       </button>
//     </nav>
//       <h1 className="text-3xl font-bold text-indigo-700 mb-6">My Reviews</h1>
//       {reviews.length === 0 ? (
...
export default function MyReviewsPage() {
  ...
  const handleDelete = async (id) => {
    if (!student) return;  // 🔥 Ensure student exists
    if (!confirm("Delete this review?")) return;
    const res = await fetch(`/api/reviews/${id}`, { method: "DELETE" });
    if (res.ok) {
      fetchReviews(student._id);  // 🔥 Use student._id
    } else {
      const result = await res.json();
      alert("❌ Delete failed: " + result.message);
    }
